...
pub enum Target10InfraError { HardwarePathUnimplemented }

pub struct Target10Adapter;
impl Target10Adapter { pub fn new() -> Self { Self }
...
impl Default for Target10Adapter { fn default() -> Self { Self::new() } }

impl MessagePublisher for Target10Adapter {
    type Message = Target10Command;
    fn publish(&self, message: Self::Message) -> Result<(), TransportError> { self.send_command(&message).map_err(|_| TransportError::Disconnected) }
}
