...
impl Default for Target5Adapter { fn default() -> Self { Self::new() } }

impl DataSource for Target5Adapter {
    type Item = Target5Status;
    fn read(&self) -> Result<Self::Item, DataSourceError> { self.poll_hardware().map_err(|_| DataSourceError::Unavailable) }
}
